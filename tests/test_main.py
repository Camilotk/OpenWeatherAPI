...
from app.main import return_if_cache_is_valid, cache_is_valid, calc_average_temperature
...
@pytest.mark.parametrize("cache_time, expected", [
    ((datetime.now() - timedelta(1)), False),
    (datetime.today(), True)
])
def test_cache_is_valid(cache_time, expected):
    assert cache_is_valid(cache_time) == expected

def test_return_cache_is_valid(cache_database):
